...
def abmag(fmjy):
    return -2.5 * np.log10(fmjy / 1000.0) + 8.9


def absmag(abm, dMpc):
    return abm - 5 * np.log10(dMpc) - 25.0
...
def drwtau_mc(embh, wav, absmag):
    ...
    taulog = (
        A
        + B * np.log10(wav / 4000.0)
        + C * (absmag + 23.0)
        + D * np.log10(embh / 1.0e9)
    )
    return 10**taulog


def drwsfinf_mc(embh, wav, absM, z=0.0):
