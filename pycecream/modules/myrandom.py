...
def unirand(no, a, b):
    dat = 1.0 * (b - a) * np.random.rand(no) + 1.0 * a
    return dat
...
def unirand_n(no, sigma, mu, a, b):
    dat_n = (
        1.0
        / (sigma**2 * 2 * pi) ** 0.5
        * e ** (-((unirand(no, a, b) - mu) ** 2) / (2 * sigma**2))
    )
...
def unirand_e(no, tau, a, b):
    ...
    dat_e = 1.0 / tau * e ** (-unirand(no, a, b) / tau)
    return dat_e
...
def normdisoldandbroken(n, mu, sigma):
    ...
    while count < n:
        ...
        randprob[:, 0] = (
            1.0
            / (sigma**2 * 2 * pi) ** 0.5
            * e ** (-((randval - mu) ** 2) / (2 * sigma**2))
        )
        randprob[:, 0] = randprob[:, 0] / np.max(
            randprob[:, 0]
        )  ##the probablility that the chosen data point will remain
