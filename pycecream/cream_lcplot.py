...
import glob
import pycecream.cream_posterior as cpos
import os
import logging
logging.basicConfig(
    level=logging.INFO,
...
def lcplot(dnow,
 ...
 for idnow in range(ndres):
  ...
  try:
   ...
   logger.info('no outlier rejection found in...',dnow+'/plots/data_echo_dat_*')
  ...
  if (plottrace == 1):
   ...
   for idx in range(ntrace):
    ...
    logger.info('traceplot...',iynow,ixnow,np.mean(ptrace_plot[idx]), np.std(ptrace_plot[idx]))
   #plt.tight_layout()
  ...
  if (postplot == 1):
   logger.info('making posterior plot....','posterior_{}{}'.format(tit[idnow],img_format))
   try:
    ...
    logger.info('unable to make covariance plot for disc posteriors. Please check at least some of these are fitted in the MCMC.')
