...
class pycecream:
    ...
    def __init__(self):
        ...
        self.fortran_compile_command = (
            self.fortran_caller + " cream_f90.f90 -o creamrun.exe"
        )
        ...
        self.lightcurve_input_params = pd.DataFrame(
            columns=[
                "name",
                "type",
                "wavelength",
                ...
                "background offset start",
                "vertical scaling start",
            ]
        )
        self.global_input_params = pd.DataFrame(
            columns=[
                "redshift",
                "BH mass",
                "BH efficiency",
                "upper fourier frequency",
                "",
            ]
        )
    ...
    def setup_directory_structure(self):
        ...
        if self.append_date_to_output_directory is True:
            dir_pycecream = (
                self.project_folder
                + "_"
                + str(pd.datetime.today().strftime("%d_%m_%Y"))
                + "_"
            )
        ...
        try:
            ...
            raise Exception(
                "directory..." + self.dir_pycecream + " already exists. "
    ...
    def add_lc(
        ...
        background_offset_start=[-1.0, -1.0],
        vertical_scaling_start=[-1.0, -1.0],
        ...
        if share_previous_lag is False:
            ...
            tophat_centroid_step = self.lightcurve_input_params[
                "tophat centroid step"
            ].values[-1]
        ...
        df = pd.DataFrame(
            ...
            index=[
                ...
                "wavelength",
                "noise model",
                "extra variance prior",
                "multiplicative errorbar prior",
                "share previous lag",
                "temporary file name",
                "mean",
                "standard deviation",
                ...
                "background offset start",
                "vertical scaling start",
                "background offset prior",
                "vertical scaling prior",
                "background_polynomials",
            ],
        ).T

        self.lightcurve_input_params = pd.DataFrame(
            pd.concat([self.lightcurve_input_params, df])
        )
        self.lightcurve_input_params["wavelength"] = pd.to_numeric(
            self.lightcurve_input_params["wavelength"], downcast="float"
        )
        self.lightcurve_input_params["mean"] = pd.to_numeric(
            self.lightcurve_input_params["mean"], downcast="float"
        )
        self.lightcurve_input_params["standard deviation"] = pd.to_numeric(
            self.lightcurve_input_params["standard deviation"], downcast="float"
        )
        self.lightcurve_input_params["tophat centroid"] = pd.to_numeric(
            self.lightcurve_input_params["tophat centroid"], downcast="float"
        )
        self.lightcurve_input_params["tophat centroid step"] = pd.to_numeric(
            self.lightcurve_input_params["tophat centroid step"], downcast="float"
        )
        self.lightcurve_input_params["tophat width"] = pd.to_numeric(
            self.lightcurve_input_params["tophat width"], downcast="float"
        )
        self.lightcurve_input_params["tophat width step"] = pd.to_numeric(
            self.lightcurve_input_params["tophat width step"], downcast="float"
        )
    ...
    def set_creamnames(self):
        ...
        for i in range(self.count_lightcurves):
            f.write(
                "'"
                + self.lightcurve_input_params["temporary file name"].values[i]
                + "' "
    ...
    def set_priors(self):
        ...
        idp = [-8, -7, -5, -6]
        dfp = [
            self.lightcurve_input_params["background offset prior"],
    ...
    def set_start_offsert_vertical(self):
        ...
        f = open(
            self.dir_pycecream + "/" + self.dir_sim + "/offsetstretch_fix.par", "w"
        )
        n = len(self.lightcurve_input_params)
        for i in range(n):
            os, os_step = self.lightcurve_input_params.iloc[i][
                "background offset start"
            ]
            v, v_step = self.lightcurve_input_params.iloc[i]["vertical scaling start"]
            f.write(
                str(os) + "," + str(os_step) + "," + str(v) + "," + str(v_step) + "\n"
            )
        f.close()
    ...
    def set_tophat(self):
        ...
        for i in range(self.count_lightcurves):
            f.write(
                ...
                + " "
                + str(
                    self.lightcurve_input_params["tophat centroid prior cent"].values[i]
                )
                + " "
                + str(
                    self.lightcurve_input_params["tophat centroid prior width"].values[
                        i
                    ]
                )
                + " "
                ...
                + " "
                + str(
                    self.lightcurve_input_params["tophat width prior width"].values[i]
                )
                + "\n"
            )
    ...
    def set_background_polynomials(self):
        ...
        if NPpoly > 0:
            f = open(
                self.dir_pycecream + "/" + self.dir_sim + "/creaminpar_bg.par", "w"
            )
            f.write(str(NPpoly) + "\n")
    ...
    def run(self, ncores=2):
        ...
        self.lightcurve_input_params.to_csv(
            "./simulation_files/input_lightcurve_settings.csv"
        )

        ## compile fortran and execute jobs across ncores
        os.system(self.fortran_compile_command)
        jobs = []
        for i in range(ncores):
            logger.info("launching job {} of {}".format(i + 1, ncores))
            p = sp.Popen(["./creamrun.exe"])
            jobs.append(p)
            time.sleep(1)
    ...
    def get_MCMC_chains(self, location=None):
        ...
        for results_dir in results_dir_list:
            ...
            p_output_names = (
                p_output_names
                + ["stretch " + l for l in lcnames]
                + ["offset " + l for l in lcnames]
                + ["noise m " + l for l in lcnames]
            )
            p_output = np.hstack(
                (p_output, dat_output[:, : 3 * self.count_lightcurves])
            )
            ...
            p_output = np.hstack(
                (p_output, dat_output[:, : 2 * self.count_lightcurves])
            )
            ...
            if polynomial_background_exists is True:
                ...
                for i in range(n_lightcurves):
                    p_pbg_names += [
                        lcnames[i] + " ng polynomial order " + str(i2 + 1)
                        for i2 in range(norder)
                    ]
            ...
            self.output_parameters = pd.concat(
                [self.output_parameters, p_output], axis=0
            )
            idx_chain += 1
    ...
    def get_MCMC_fourier_chains(self, location=None):
        ...
        for results_dir in results_dir_list:
            if idx_chain == 1:
                ...
                fourier_info = np.loadtxt(
                    results_dir + "/cream_gvalues.dat", skiprows=1
                )
                self.fourier_info = pd.DataFrame(
                    fourier_info,
                    columns=[
                        "angular frequency",
                        "prior std",
            ...
            dat_fourier = pd.DataFrame(
                np.loadtxt(results_dir + "/CREAM_allpars_BIG.dat")[:, : 2 * Nfreq]
            )
    ...
    def get_light_curve_fits(self, location=None):
        ...
        for results_dir in results_dir_list:
            ...
            output_merged_uncertainties_chains += [
                np.array(output_merged_uncertainties).T
            ]

        names = list(self.lightcurve_input_params["name"])
        y = np.mean(np.array(output_merged_model_chains), axis=0)
        sig = (
            np.sqrt(np.sum(np.array(output_merged_uncertainties_chains) ** 2, axis=0))
            / ncores
        )
        self.output_merged_model = {
            "time": dat[:, 0],
    ...
    def plot_results(self, location=None):
        ...
        simulation_dir = self.get_simulation_dir(location=location)
        self.lightcurve_input_params = pd.read_csv(
            simulation_dir + "/simulation_files/input_lightcurve_settings.csv"
        )
        ...
        cream_plot.lcplot(
            os.path.join(simulation_dir, "simulation_files"),
    ...
    def get_flux_flux_analysis(self, plotfile=None, location=None, xlim=[-4, 4]):
        ...
        for c in cols:
            ...
            try:
                fit = np.polyfit(
                    driver_d_itp, flux_d, w=1.0 / sig_d**2, deg=1, cov=True
                )
...
class dream:
    ...
    def add_lc(self, lc, name, errorbar_variance=True, errorbar_rescale=True):
        ...
        self.pc.add_lc(
            ...
            share_previous_lag=sharelag,
            tophat_centroid_step=0.0,
        )
        self.numlc += 1
    ...
    def _merge_use_model_residue(self):
        ...
        idxmod = np.where(
            (model["time"].values > tmin) & (model["time"].values < tmax)
        )[0]
    ...
    def __combine_individual_output_lightcurves(self):
        ...
        return {
            "combined_input": datinput[np.argsort(datinput[:, 0]), :],
    ...
    def plot_chains_or_covariances(
        self, output_chains, Parm, ParmNicename, type="covarainces", fig_in=None
    ):
        """
        ...
        dfstd = df.std()
        names_VariedColumns = list(dfstd[dfstd > 1.0e-10].index)
