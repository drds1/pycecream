...
def corner_1(xs, weights=None, labels=None, extents=None, truths=None,
           ...
           fig=None,header=[],sigconts=[100.-68,100.-95,100-99.7],figname='histplot.pdf',skip=1,xti=[-1],yti=[-1], annotate=[],plot_corr=0,**kwargs):
    ...
    annotate : list[[...nj]....ni] of strings to add to each plot
    
    sigconts : plot contour levels by default at 1, 2, 3 sigma... e.g [100.-68,100.-95,100-99.7]
    ...
    for i, x in enumerate(xs):
        ...
        for j, y in enumerate(xs):
            ...
            if (len(annotate) > 0):
             ax.text(0.95,0.9,annotate[i][j],ha='right',transform=ax.transAxes)
...
def hist_1(xs, labels=None, lims=None, truths=None,nbins=[50],colin ='b',truth_color = 'r',annotate = None, figname = 'histplot.pdf'):
 ...
 ax1.hist(xs,bins = nbins[0], histtype='step',color = colin)
 
 if (annotate != None):
  ax1.text(0.95,0.9,annotate,ha='right',transform=ax1.transAxes)
