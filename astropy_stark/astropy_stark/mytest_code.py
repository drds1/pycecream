...
wavmin=0#1*10**-10#const.w_c/T[1]
wavmax=1*10**-3#const.w_c/T[size-1]#1*10**-6#
...
if output == 1:
	...
	xticks(fontsize=15)
	yticks(fontsize=15)
	title ('Z = '+str(text[0])+' .  r_in/r_out (pc)=3R_s /' +str(text[2])+'.  M='+str(text[4])+'.  M dot='+str(text[3])+'.  Inc='+str(text[5]))
	show()
